...
# ------------------
    ...
    for visibility in wdata:
        ...
        if vtime < Utstart[0]:
            ...
            if this_ui!=ui:
                ui = this_ui
                if ui:
                    print time.ctime().split()[3]': Processing chunk'
    ...
    for name in intl_name:
        nlist,ncube = np.sort(glob.glob('FR_*'_+name+'_*.fits')), np.array([])
        for nfile in nlist:
            try:
                ncube = dstack((nmap, getdata(nfile)))
            except:
                ncube = np.copy(getdata(nfile))
        h = getheader(nfile)
        ncomb = imcomb (amap)
        thisname = './FR_'+aipsname+' '+name+'.fits'
        writeit (ncomb,h['CRVAL1'],h['CRVAL2'],h['NAXIS1'],h['NAXIS1']*h['CDELT2'],\
                 thisname)
        pload (thisname,aipsname,indisk,'IMAP',logfiledir=logfiledir,doindxr=False)
